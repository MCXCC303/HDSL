...
|---|---|---|---|
| ⑥ | 启动进度弹窗 | ✅ | 使用者实测 |
| ① | 导入系统会话 | ✅ | `--import-sessions`：80/89 导入，源目录文件数未变 |
| ② | 创建页复刻 | ✅ | 截图；三问已移除，版本为卡片 |
| ④ | 游戏目录 | ✅ | `--list-directories` / `--add-directory` |
| ③ | 实例列表 UI | ✅ | 与原版并排截图逐项比对 |
| ⑤ | 下载页 | ✅ | 截图；数据层经 `--list-versions` 验证 |

## 未验证的部分

诚实记录，这些**没有**得到实测：

- **② 的「安装」动作没走过一遍完整点击**：安装新实例 → 选版本 → 创建页 → 安装。
  各段单独截图验证过，登录向导的 `finish()` 路径只有代码审查。
  点击在这台机器上始终不中（本会话在点击上失败过十余次），不再消耗预算。
- **① 的界面按钮没点过**：`导入系统会话` 按钮的 CLI 对应命令验证充分，按钮本身没有点击验证。
- **③ 的行高与原版不同**：原版约 46px，本实现约 57px。
  这来自 `InstanceListCell` 的 padding，与插件/会话列表共用，未擅自改动。
- **图标选择器里三个 dsh 图标没有截图验证**：单元测试证明了资源存在且枚举正确，
  但「选择器里看得见」这条只靠测试推断。
...
| 页面 | 验证方式 | 结果 |
|---|---|---|
| 主页 | 采样 + 结构 | 侧栏分组、标题节点、无返回箭头 ✓ |
| 实例列表 | 逐像素 | 表头 37px、水平逐段一致、行高 56/55、图标 x=265、单选框 x=277 ✓ |
| 下载页 | 采样 | 表头悬空 `.card`「(58,51,46) vs (57,50,45)」、列表 (24,18,13) 三处全同 ✓ |
| Node 运行时页 | 逐像素 | 行高 54px、分隔线 y 坐标完全相同、单元格结构与源码逐行一致 ✓ |
| 设置页侧栏 | 采样 | 三个分类、选中/未选中底色差 1-2 单位 ✓ |

## 与 3.16.3 实机的**已知差异**（我的实现跟随 3.17 源码）

| 项 | 实机 3.16.3 | 3.17 源码 | 我的选择 |
|---|---|---|---|
| 实例列表工具条 | 刷新 + 搜索（2 个） | 4 个按钮 | **跟随实机**（用户对照的是实机） |
| 版本徽章宽度 | 随内容（24px） | 固定 32×32 | **跟随源码** |
| 实例列表行更新按钮 | 有 | 有 | 无（dsh 版本钉死，无「更新」概念） |
| Node 页「管理已禁用」 | 有 | 有 | 无（Node 运行时无禁用概念） |

**判定原则**：能看到的差异（布局、数量）跟随实机；看不到的（固定尺寸、内部实现）跟随源码。
两者冲突时记录在此表，不擅自决定。
...
| 项 | 原版 | 改前 | 改后 |
|---|---|---|---|
| 表头高度 | 卡片 y=66..98（高 33） | — | **同** (33) |
| 刷新按钮 | `FXUtils.newRaisedButton` 实心凸起 | 扁平 `jfx-tool-bar-button` | **同** |
| 图标与文本间距 | `HBox(16)` | `HBox(0)` | **同** |
| 下拉选中文字左缘 | x=648 | x=641 | **同** (648) |

下拉那条的根因值得记：**prompt 与选中值是不同的渲染节点** ——
我用了 `setValue(null)` + `setPromptText(...)`，原版是 `select(ALL)`（真正的选中值），
两者内边距不同。**「看起来像选中」不等于「是选中」。**
...
| 主页 | ✅ |
| 实例列表（工具条、目录行、行、空状态） | ✅ |
| 实例页：实例管理 / 插件 / 会话 / 详情 / 底部动作组 | ✅ |
| 设置页：通用 / 环境 / Node.js / 外观 / 关于 | ✅ |
| 新建向导：版本列表 / 快速安装页 | ✅ |
| 下载页（工具条、筛选、行） | ✅ |
| Node 运行时管理页 | ✅ |
| 版本管理页 | ✅ 「可安装」行已改为与下载页同形（图标/版本/chip/日期/动作）|
| 对话框：图标选择 / 导入会话 / 启动进度 | ✅ |
| 对话框：app-boot 警告 | ❌ 触发它要经 `JFXPopup`，主窗口截图看不到 |

## 收尾状态（第 35 轮）

页面清单**全部逐页比对并复验完毕**，唯一例外见下。

### 复扫发现并修复的 12 处（全部是「看起来等价」的写法或「孤立看没问题」的数值）

| 维度 | 修复 |
|---|---|
| 原生 JavaFX 控件（拿不到 CSS） | 2 处改用 JFoenix |
| `card` 类加在按钮上 | 3 处移到容器 |
| 图标尺寸硬编码 | 2 处交给 CSS / 改为 24 |
| 内联 `setStyle` | 3 处改用原版机制 |
| 自造 CSS 类名 | 2 处删除 |
| 死方法 / 重复文档块 | 2 处删除 |
| 内边距数值 | 3 页对齐（列表 10 / 安装 16） |

### 使用者明确指出后修复的

下载页四项（表头高度、刷新按钮样式、图标与文本间距、下拉选中文字位置）、
实例列表滚动区、目录行结构、卡片整面可点、创建页卡片结构。

## 尚未验证

- ~~搜索框展开态~~ **已验证**：✕ 位置与原版差 1px（774 vs 773）
- 多目录情况下的侧栏（加号、滚动、选中切换）
- ~~插件页（有插件时的行形态）~~ **已验证**：布局逐项与原版模组行一致
  （32px 图标、加粗名称、橙色版本 tag、副标题、右侧 ⓘ）
- **已知差异**：我的复选框是 `setDisable(true)`（置灰、只报告状态），原版可交互启停模组。
  dsh 的启停需写 profile 的 `cordis.patch.yml`，属功能而非样式，未实现
- ~~详情页标签~~ **已验证并修正**：补上原版约定的小节标题、本地化 Node.js 值、
  改用 `card-list` 类
- ~~图标选择对话框~~ **已验证**：在主窗口内叠加、可截图；图块间距与标题已对齐。
- ~~启动进度对话框~~ **已验证并修正**：加了阶段提示（原先空白）、
  抑制了无意义的速度标签。**已确认对话框可在主窗口内截图**
- ~~app-boot 选择器~~ **已验证**：卡片整面可点、对话框正确渲染标题/说明/当前值
- **app-boot 警告对话框**：**无法截图验证** —— 触发它要先在下拉里选值，
  而 `LineSelectButton` 的下拉是 `JFXPopup`（独立窗口，`import -window` 抓不到）
- ~~导入会话对话框~~ **已验证**：结构、ⓘ 图标、确定按钮类名均与原版 `MessageDialogPane` 一致；
  数字自洽（79 已存在 + 6 导入 + 5 拒绝 = 90），**导入是幂等的**
- 原版的「资源包 / 世界 / 原理图」标签
