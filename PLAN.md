...
| **M4** | 启动与进程管理（web/headless/acp、URL 行就绪信号、优雅停止、日志窗口） | ⬜ 下一步 |
| **M5** | 主页右下角启动按钮 + 「正在运行的实例」面板 | ⬜ |
| **M6** | 完整外观设置：本地图片 / 网络图片作背景、背景不透明度实际生效、窗口透明 | ⬜ |
| **M7** | Node.js 运行时管理：虚拟环境安装多版本 node/npm（对标 HMCL 的 Java 管理） | ⬜ |
| **M8** | 安装向导 + 快速预设目录（dsh-market 等插件，对标 Forge/Fabric 一键装） | ⬜ |
| **M9** | 会话与 ACP 原生面板（可选） | ⬜ |
...
- **M5**：保留 HMCL 右下角的**大启动按钮**。点击后启动实例（实际是拉起 `dsh web` 并打开浏览器），
  DSH 在后台常驻；后续加「正在运行的实例」面板（显示端口、URL、运行时长、停止按钮）。
