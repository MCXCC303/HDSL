...
```
Insets(8)  × 20     行单元格、小控件
Insets(10) × 14     列表页根部、卡片外边距   ← 下载页 / 版本页 / 设置页
Insets(16) × 2      安装向导页根部           ← AbstractInstallersPage
Insets(20) × 1      仅主页的启动面板
```

自造页面时按这个选：**列表/设置页用 10，安装向导页用 16，只有主页那种大面板才用 20**。
用错会让整页内容比原版内缩一圈 —— 而且因为整页都缩，很难看出来。


原版的写法是 **一个小节装若干项**，小节名比里面的行名**更宽**：

```
外观                      ← 小节：装六项
  主题模式 / 主题色 / 主题色风格 / 背景 / 标题栏透明 / 窗口透明
主题                      ← 小节：装两项
  主题 / 导出
```

我给**每一项都开了一个小节**，于是节名与行名并排重复（字体/字体、动画/动画）。

**判定方法**：若某小节的标题与它唯一那行的标题是同一个词，就是分组错了。

（例外：原版的「主题」小节里确实有一行也叫「主题」—— **先查原版有没有同样的重复**，
有就保留，别自作主张改掉。）
